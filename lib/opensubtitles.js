...
async function fetchSrt(imdbId, language, season, episode) {
  ...
  // Find the first subtitle matching the requested language
  const match = allSubs.find((sub) => sub.lang === langCode);

  if (!match) {
    console.log(`[DualSubs] No subtitle found for lang=${langCode}`);
    return null;
  }

  console.log(`[DualSubs] Found subtitle id=${match.id} for lang=${langCode}`);
